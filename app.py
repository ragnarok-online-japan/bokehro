...
def bokehro():
    ...
    refining_list: list[int] = request.args.getlist("refining[]", type=int)
    ...
    if item_name is not None:
        try:
            ...
            query_item_detail: str = """
                SELECT id, world, datetime, {:s} AS 'unit_cost', refining, cards, enchants
            ...
            refining_list = [value for value in refining_list if isinstance(value, int) == True]
            if len(refining_list) > 0:
                query_item_detail += " AND refining IN({:s})".format(",".join(map(str, refining_list)))
...
@app.route('/bokehro-dev', methods=['GET'])
def bokehro_dev():
    item_name: str           = request.args.get("name", default="")
    is_card: str             = request.args.get("is_card", default="_none_")
    is_enchant: str          = request.args.get("is_enchant", default="_none_")
    is_round_cost: bool      = request.args.get("is_round_cost", default=True, type=bool)
    refining_list: list[int] = request.args.getlist("refining[]", type=int)
    ...
    if item_name is not None:
        try:
            ...
            query_item_detail: str = """
                SELECT id, datetime, {:s} AS 'unit_cost', refining, cards, enchants
                FROM item_detail_tbl
                WHERE item_name = %(item_name)s
            """.format(sql_unit_cost)
